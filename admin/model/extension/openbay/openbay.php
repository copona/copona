...
class ModelExtensionOpenBayOpenbay extends Model {
    ...
    public function updateV2Test() {
        ...
        if (!$this->error) {
            ...
            return array( 'error'            => 0, 'response'         => '', 'percent_complete' => 20,
                'status_message'   => $this->language->get('text_check_new') );
        } else {
        ...
        if ($this->lasterror == true) {
            ...
            if ($data['version'] > $current_version) {
                ...
                return array( 'error'            => 0, 'response'         => $data['version'],
                    'percent_complete' => 40,
                    'status_message'   => $this->language->get('text_downloading') );
            } else {
    ...
    public function updateV2Download($beta = 0) {
        ...
        return array( 'error'            => 0, 'response'         => $curl_error,
            'percent_complete' => 60,
            'status_message'   => $this->language->get('text_extracting') );
    }
    ...
    public function updateV2Extract() {
        ...
        if (!$this->error) {
            return array( 'error'            => 0, 'response'         => '', 'percent_complete' => 80,
                'status_message'   => $this->language->get('text_remove_files') );
        } else {
            return array( 'error' => 1, 'response' => $this->error );
        }
    }
    ...
    public function updateV2Remove($beta = 0) {
        ...
        if (!$this->error) {
            return array( 'error'            => 0, 'response'         => '', 'percent_complete' => 90,
                'status_message'   => $this->language->get('text_running_patch') );
        } else {
            ...
            return array( 'error'            => 1, 'response'         => $response_error,
                'percent_complete' => 90,
                'status_message'   => $this->language->get('text_running_patch') );
        }
    }
    ...
    public function updateV2UpdateVersion($beta = 0) {
        ...
        if ($this->lasterror == true) {
            ...
            return array( 'error'            => 0, 'response'         => $data['version'],
                ...
                'status_message'   => $this->language->get('text_updated_ok') . $data['version'] );
        }
    }
            ...
            if (@ftp_login($connection, $data['user'], $data['pw'])) {
                ...
                if ($folder_error_admin == true) {
                    ...
                    if ($folder_error == true) {
                        return array( 'connection' => false, 'msg'        => $this->language->get('error_no_files'),
                            'dir'        => json_encode($directory_list) );
                    } else {
    ...
    public function update() {
        ...
        if ($connection != false) {
            ...
            if (@ftp_login($connection, $data['user'], $data['pw'])) {
                ...
                $send = array( 'version'   => $current_version, 'ocversion' => VERSION,
                    'beta'      => $data['beta'] );
                ...
                if ($this->lasterror == true) {
                    ...
                    foreach ($files['asset']['file'] as $file) {
                        ...
                        $filedata = base64_decode($this->call('update/getFileContent/', array(
                                'file' => implode('/', $file['locations']['location']) . '/' . $file['name'],
                                'beta' => $data['beta'] )));
                ...
                return array( 'connection' => true, 'msg'        => sprintf($this->language->get('text_updated'), $files_update['version']),
                    'version'    => $files_update['version'] );
            } else {
                return array( 'connection' => false, 'msg' => $this->language->get('error_ftp_login') );
            }
        } else {
