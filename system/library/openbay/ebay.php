...
final class Ebay {
    ...
    public function getEbayItemList($limit = 100, $page = 1, $filter = array()) {
        ...
        return $this->call('item/getItemListLimited/', array( 'page'   => $page,
                'limit'  => $limit,
                'filter' => $filter ));
    }
        ...
        if ($listing['status'] == 1) {
            ...
            if ($sku == null) {
                ...
                if ($stock <= 0) {
                    ...
                    $this->call('item/reviseStock/', array( 'itemId' => $item_id,
                        'stock'  => $stock ));
                        ...
                        foreach ($ebay_listings[$item['itemId']]['variants'] as $ebay_variant) {
                            ...
                            if (in_array($ebay_variant['sku'], $var_ids)) {
                                ...
                                if ($ebay_variant['qty'] != $options[$option_id]['stock']) {
                                    ...
                                    $this->call('item/reviseStock/', array( 'itemId' => $item['itemId'],
                                        'stock'  => $options[$option_id]['stock'],
    ...
    public function relistItem($item_id, $product_id, $qty) {
        ...
        $response = $this->call('listing/relistItem/', array( 'itemId' => $item_id,
            'qty'    => $qty ));
        ...
        if ($reserve == 0) {
            ...
            if ($this->getReserve($product_id, $item_id, $sku) != false) {
                ...
                if ($variant == 0) {
                    ...
                    $this->addReserve(array( 'product_id' => $product_id, 'qty'        => array(
                            0 => $reserve ) ), $item_id, 0);
                } else {
                    $this->log('updateReserve() - variant');
                    $this->addReserve(array( 'product_id' => $product_id, 'opt'        => array(
                            array(
