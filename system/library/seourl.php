...
class Seourl
{
    ...
    public function convertAccentsAndSpecialToNormal($string)
    {
        ...
        return strtolower(trim($new_string,"- "));
    }
    ...
    public function uniqueSeoKeyword($key = '', $language_id = '', $query = '', $table_name = 'url_alias')
    {
        ...
        $result = $this->db->query("select * FROM " . DB_PREFIX . "$table_name WHERE $language_sql $query_sql keyword = '" . $this->db->escape($key) . "'");
        $i = 0;
        while ($result->num_rows > 0) {
            $i++;
            $nextKeyword = $key . "-" . $i;
            $result = $this->db->query("select * FROM " . DB_PREFIX . "$table_name WHERE $language_sql $query_sql keyword = '" . $this->db->escape($nextKeyword) . "'");
    ...
    {
        $result = $this->db->query("SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query='" . $this->db->escape($query) . "'");

        if($result->num_rows){
            return $result->row['keyword'];
            //return [ 'count' => $result->num_rows,'keyword' => $result->row['keyword'] ];
        } else {
            //return [ 'count' => 0,'keyword' => false ];
            return false;
        }
    ...
    {
        $result = $this->db->query("SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE keyword='" . $this->db->escape($keyword) . "'");
        return $result->num_rows;
    }


    public function setUrlAlias( $query, $keyword, $table_name = 'url_alias' ) {

        // Deleting
        $this->db->query( "DELETE from " . DB_PREFIX . "$table_name
        WHERE query = '" . $this->db->escape($query) . "'");

        $sql = "REPLACE INTO " . DB_PREFIX . "$table_name set
