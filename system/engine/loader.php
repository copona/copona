...
final class Loader {
	...
	public function model($route) {
		...
		if (!$this->registry->has('model_' . str_replace(array( '/', '-', '.' ), array(
			...
			if (is_file($file)) {
				...
				$this->registry->set('model_' . str_replace(array( '/', '-', '.' ), array( '_',
						'', '' ), (string)$route), $proxy);
			} else {
	...
	public function view($route, $data = array()) {
		...
		// Trigger the pre events
		$result = $this->registry->get('event')->trigger('view/' . $route . '/before', array(
			&$route, &$data, &$output ));
		...
		if (!$output) {
			$template = new Template($this->registry->get('config')->get('template_type'), $this->registry);
		...
		// Trigger the post events
		$result = $this->registry->get('event')->trigger('view/' . $route . '/after', array(
			&$route, &$data, &$output ));
