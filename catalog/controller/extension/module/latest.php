        ...
        if ($results) {
            foreach ($results as $result) {
                ...
                if ($result['image']) {
                    ...
                    $popup = $this->model_tool_image->{Config::get('theme_default_latest_thumb_resize')}(Config::get('config_no_image', 'placeholder.png'),
                ...
                if ($result['product_group_id']) {
                    ...
                    foreach ($group_products as $group_product) {
                        ...
                        if ($group_product['image']) {
                            $group_image = $this->model_tool_image->cropsize($group_product['image'], $this->config->get($this->config->get('config_theme') . '_image_product_group_width'),
                              $this->config->get($this->config->get('config_theme') . '_image_product_group_height'));
                        } else {
                            $group_image = $this->model_tool_image->cropsize(Config::get('config_no_image', 'placeholder.png'), $this->config->get($this->config->get('config_theme') . '_image_product_group_width'),
                              $this->config->get($this->config->get('config_theme') . '_image_product_group_height'));
                        }
                ...
                $data['products'][] = array(
                  ...
                  'popup'                => $popup,
                  'name'                 => $result['name'],
                  'stripped_description' => utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0,
