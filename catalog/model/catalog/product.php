...
class ModelCatalogProduct extends Model
{
    ...
    public function getProduct($product_id)
    {
        ...
        $sql = "SELECT DISTINCT p.*, p2p.product_group_id, p2pd.product_id as default_group_product_id, "
          ...
          . "(SELECT cm.value FROM " . DB_PREFIX . "content_meta cm WHERE cm.content_type = 'product' AND content_id = '" . (int)$product_id . "') AS content_meta, "
        ...
        if ($query->num_rows) {
            return array(
              ...
              'price' => round(($query->row['discount'] ? $query->row['discount'] : $query->row['price']), 2),
              'special' => round($query->row['special'], 2),
